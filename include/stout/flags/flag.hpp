...
struct Warning {
 public:
  explicit Warning(const std::string& _message)
    : message(_message) {}
...
struct Flag {
  ...
  // This is the name that the user uses to specifically load the flag (e.g,
  // via command line `--foo=val`). This is optional because a flag might not
  // be explicitly loaded by the user (e.g., flag with a default value). Note
  // that this name should be one of `name` or `alias`.
