...
// `WindowsFD`. The `os::*` functions return a type appropriate to the
// platform, which allows us to write code like this:
//
...
class WindowsFD {
 public:
  ...
  explicit WindowsFD(intptr_t socket)
    : WindowsFD(static_cast<SOCKET>(socket)) {}
  ...
  WindowsFD(int crt)
    : WindowsFD(INVALID_HANDLE_VALUE) {
    if (crt == -1) {
  ...
  operator HANDLE() const {
    ...
    //
    // [1]: https://tinyurl.com/2dudkrp9 // NOLINT(whitespace/line_length)
    if (type() == Type::SOCKET) {
  ...
  //
  // [1]: https://tinyurl.com/nucstyvh // NOLINT(whitespace/line_length)
